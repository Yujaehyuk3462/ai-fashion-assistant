...
commit `c4704db` 이후, 아직 커밋되지 않은 작업까지 포함한 전체 작업 내역 정리.
...
- commit `c4704db`까지는 커밋 완료.
- 그 이후 (스크랩 기능, Gemini 모델 폴백, 오버플로 수정) 전부 **아직 커밋 안 됨** — 사용자의 명시적 커밋 요청 대기 중.
- 미커밋 변경 파일: `firestore.rules`, `lib/screens/fitting_room_screen.dart`, `lib/screens/home_screen.dart`, `lib/screens/settings_screen.dart`, `lib/screens/wardrobe_screen.dart`, `lib/services/firestore_service.dart`, `lib/services/fitting_job_controller.dart`, `lib/services/gemini_service.dart`
- 신규 미추적 파일: `lib/models/scrap_entry.dart`, `lib/screens/scrap_screen.dart`, `lib/widgets/`(full_screen_image_viewer.dart 포함)
